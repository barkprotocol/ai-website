        ...
        if (!config) {
          const header = (
            <div className="flex min-w-0 flex-1 items-center gap-2">
              <div className={cn("h-1.5 w-1.5 rounded-full bg-destructive ring-2 ring-destructive/20")} />
              <span className="truncate text-xs font-medium text-foreground/90">Tool Error</span>
          ...
          return (
            <div key={toolCallId} className="group">
              <ToolResult
                ...
                result={{
                  result: "Tool Error",
                  error: "An error occurred while processing your request, please try again or adjust your phrasing.",
                }}
                header={header}
              />
            </div>
...
function ChatMessage({ message, index, messages, setSavedPrompts, onPreviewImage, addToolResult }: ChatMessageProps) {
  ...
  return (
    ...
    >
      ...
      <div className="group relative flex max-w-[85%] flex-row items-center">
        ...
        <div className={cn("relative gap-2", isUser ? "items-end" : "items-start")}>
          {hasAttachments && (
