...
import { IS_SUBSCRIPTION_ENABLED, IS_TRIAL_ENABLED, cn, getSubPriceFloat, getTrialTokensFloat } from "@/lib/utils"
...
export function HomeContent() {
  ...
  const [isFetchingSavedPrompts, setIsFetchingSavedPrompts] = useState<boolean>(true)
  ...
  const handleSend = async (value: string, attachments: Attachment[]) => {
    const NON_TRIAL_PERMISSION = !user?.earlyAccess && !user?.subscription?.active
    const TRIAL_PERMISSION = !user?.earlyAccess && !user?.subscription?.active && !meetsTokenBalance
  ...
  const filteredPrompts = input.startsWith("/")
    ? savedPrompts.filter((savedPrompt) => savedPrompt.title.toLowerCase().includes(input.slice(1).toLowerCase()))
    : savedPrompts
  ...
  const shouldCheckPortfolio = IS_TRIAL_ENABLED && !hasEAP && !user?.subscription?.active
  ...
  const meetsTokenBalance = useMemo(() => {
    ...
    // Find the BARK token
    const neurToken = portfolio.tokens.find((token) => token.mint === process.env.NEXT_PUBLIC_BARK_MINT)
  ...
  const RENDER_TRIAL_BANNER =
    IS_TRIAL_ENABLED && !hasEAP && !user?.subscription?.active && !meetsTokenBalance && showTrialBanner
  const USER_HAS_TRIAL = IS_TRIAL_ENABLED && !hasEAP && !user?.subscription?.active && meetsTokenBalance
  const RENDER_SUB_BANNER = !hasEAP && !user?.subscription?.active && !RENDER_TRIAL_BANNER && !USER_HAS_TRIAL
  const RENDER_EAP_BANNER = !IS_SUBSCRIPTION_ENABLED && !hasEAP && !RENDER_TRIAL_BANNER && !USER_HAS_TRIAL
              ...
              <BlurFade delay={0.3}>
                <div className="space-y-2">
                  ...
                  {isFetchingSavedPrompts ? (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {savedPrompts.slice(0, Math.min(4, savedPrompts.length)).map((savedPrompt, index) => (
                        <SuggestionCard
                          id={savedPrompt.id}
                          useSubtitle={true}
            ...
            <Card className="relative max-h-full border-white/[0.1] bg-white/[0.02] p-4 backdrop-blur-sm backdrop-saturate-150 dark:bg-black/[0.02] sm:p-8">
              ...
              <div className="relative space-y-6">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  {IS_TRIAL_ENABLED && <Button onClick={toggleTrialBanner}>View Trial</Button>}
                  <Button
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center">
          <div className="mx-auto max-h-screen max-w-xl overflow-y-auto p-6">
            <Card className="relative max-h-full border-white/[0.1] bg-white/[0.02] p-4 backdrop-blur-sm backdrop-saturate-150 dark:bg-black/[0.02] sm:p-8">
              ...
              <div className="relative space-y-6">
                <div className="space-y-2 text-center">
                  <h2 className="text-lg font-semibold sm:text-2xl">Subscription Required</h2>
                  <div className="text-muted-foreground">
                    Subscribe to Bark for <Badge>BETA</Badge> access.
                  </div>
                </div>

                <div className="rounded-lg bg-white/[0.01] p-4 backdrop-blur-sm dark:bg-black/[0.01]">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text:xs font-medium sm:text-sm">Monthly Subscription</span>
                    <span className="text-base font-semibold sm:text-lg">{getSubPriceFloat()} SOL</span>
                  </div>
                </div>
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center">
          <div className="mx-auto max-h-screen max-w-xl overflow-y-auto p-6">
            <Card className="relative max-h-full border-white/[0.1] bg-white/[0.02] p-4 backdrop-blur-sm backdrop-saturate-150 dark:bg-black/[0.02] sm:p-8">
              ...
              <div className="relative space-y-6">
                <div className="space-y-2 text-center">
                  <h2 className="text-lg font-semibold sm:text-2xl">Bark Holder Trial</h2>
                  <div className="text-muted-foreground">
                    Hold $BARK tokens in your embedded wallet for <Badge>BETA</Badge> access. Deposit BARK tokens to
                    your active embedded wallet to continue.
                  </div>
                </div>

                <div className="rounded-lg bg-white/[0.01] p-4 backdrop-blur-sm dark:bg-black/[0.01]">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text:xs font-medium sm:text-sm">BARK Tokens Required</span>
                    <span className="text-base font-semibold sm:text-lg">{getTrialTokensFloat()} BARK</span>
                  </div>
                </div>
