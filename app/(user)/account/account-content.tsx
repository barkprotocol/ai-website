...
import { formatPrivyId, formatUserCreationDate, formatWalletAddress, truncate } from "@/lib/utils/format"
...
export function AccountContent() {
  ...
  if (walletsError) {
    return <div className="p-4 text-sm text-red-500">Failed to load wallets: {walletsError.message}</div>
  }
  ...
  const privyWallets = embeddedWallets.filter((w: EmbeddedWallet) => w.walletSource === "PRIVY" && w.chain === "SOLANA")
  ...
  const linkedSolanaWallet = allUserLinkedAccounts.find(
    (acct): acct is WalletWithMetadata =>
      acct.type === "wallet" && acct.walletClientType !== "privy" && acct.chainType === "solana",
  )
        ...
        <div className="max-w-3xl space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-4">
                    <Avatar className="h-10 w-10 rounded-lg">
                      ...
                      />
                      <AvatarFallback className="rounded-lg bg-sidebar-accent">{avatarLabel}</AvatarFallback>
                    </Avatar>
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Member since {userData.createdAt}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex h-8 items-center">
                        <span className={cn("text-sm font-medium")}>{user?.earlyAccess ? "Active" : "Not Active"}</span>

                        {!user?.earlyAccess && !IS_SUBSCRIPTION_ENABLED && (
                          <div className="ml-auto">
                            <Button variant="default" size="sm" onClick={() => router.push("/home")}>
                              Get Early Access
                            </Button>
                          </div>
                        )}
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>Enable Degen Mode to skip confirmation prompts for on-chain actions</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          <span className="hidden sm:inline">
                            {linkedSolanaWallet?.address ? linkedSolanaWallet?.address : "Not connected"}
                          </span>
                          <span className="sm:hidden">
                            {linkedSolanaWallet?.address ? truncate(linkedSolanaWallet?.address) : "Not connected"}
                          </span>
                        </p>
                      </div>
                    </div>
                    <Button
                      ...
                      className={cn(
                        "min-w-[100px] text-xs",
                        linkedSolanaWallet?.address && "hover:bg-destructive hover:text-destructive-foreground",
                      )}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={userData.twitter ? () => unlinkTwitter(userData.twitter!.subject) : linkTwitter}
                      className={cn(
                        "min-w-[100px] text-xs",
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{userData.email || "Not connected"}</p>
                      </div>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={userData.email ? () => unlinkEmail(userData.email!) : linkEmail}
                      className={cn(
                        "min-w-[100px] text-xs",
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={userData.discord ? () => unlinkDiscord(userData.discord!.subject) : linkDiscord}
                      className={cn(
                        "min-w-[100px] text-xs",
                        userData.discord && "hover:bg-destructive hover:text-destructive-foreground",
                      )}
                    >
          ...
          {user?.earlyAccess && IS_SUBSCRIPTION_ENABLED ? (
                ...
                <CardContent className="pt-6">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div>
                          <p className="text-sm">EAP Status - subscription not required ���️</p>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <SubscriptionSection
              ...
              nextPaymentDate={
                user?.subscription?.nextPaymentDate ? new Date(user?.subscription?.nextPaymentDate) : undefined
              }
