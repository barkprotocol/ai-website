...
import type { FilterOption, FilterValue, PromptAction } from "./types/prompt"
  ...
  const secondaryFilteredPrompts = useMemo(() => {
    ...
    return searchTerm !== ""
      ? primaryFilteredPrompts.filter((prompt) => {
          return prompt.title.toLowerCase().includes(searchTerm) || prompt.content.toLowerCase().includes(searchTerm)
        })
      : primaryFilteredPrompts
  }, [search, primaryFilteredPrompts])

  function sortPrompts(prompts: SavedPrompt[], property: keyof SavedPrompt, swapComparison = false) {
    prompts.sort((a, b) => {
      const dateA = a[property] && typeof a[property] !== "boolean" ? new Date(a[property]).getTime() : 0

      const dateB = b[property] && typeof b[property] !== "boolean" ? new Date(b[property]).getTime() : 0
  ...
  async function handleDeletePrompt() {
    ...
    toast.promise(
      deleteSavedPrompt({ id: promptAction.id }).then(() => {
        setSavedPrompts((old) => old.filter((element) => element.id !== promptAction.id))
  ...
  async function handleEditPrompt() {
    ...
    toast.promise(
      editSavedPrompt({
        ...
        setSavedPrompts((old) =>
          old.map((element) => (element.id === id ? { ...element, title: title, content, updatedAt } : element)),
        )
  ...
  async function handleAddToFavorites(id: string, isFavorite: boolean) {
    toast.promise(
      setIsFavoriteSavedPrompt({ id, isFavorite }).then((res) => {
        ...
        setSavedPrompts((old) => old.map((element) => (element.id === id ? { ...element, isFavorite } : element)))

        resetPromptAction()
      }),
      {
        loading: isFavorite ? "Adding to favorites..." : "Removing from favorites...",
        success: isFavorite ? "Prompt added to favorites" : "Prompt removed from favorites",
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col items-center gap-4 px-4 py-6 md:py-12">
      ...
      {savedPrompts.length === 0 ? (
        isLoading ? (
          ...
          <div className="flex items-center justify-center gap-2 pt-20">No prompts saved yet</div>
        )
      ) : secondaryFilteredPrompts.length === 0 ? (
        <div className="flex items-center justify-center gap-2 pt-20">No match found</div>
      ) : (
          ...
          {secondaryFilteredPrompts.map((prompt) => (
            ...
            >
              <div className="flex w-full flex-row items-center justify-between text-base font-medium">
                <p className="text-sm font-medium md:text-base">{prompt.title}</p>
              ...
              <div className="text-xs text-muted-foreground/80">{prompt.content.trim().slice(0, 150) + "..."}</div>
            </motion.div>
          ))}
        </div>
      )}
