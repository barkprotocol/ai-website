...
import { MAX_CHARS } from "@/app/(user)/home/conversation-input"
...
export const EditPromptDialog = ({
  ...
  >
    <DialogTrigger asChild>
      <Button onClick={() => updatePromptAction({ action: "save", id: null })} variant="secondary" disabled={isLoading}>
        Add Prompt <Plus className="h-4 w-4" />
      </Button>
    </DialogTrigger>
    <DialogContent className="sm:max-w-[650px]">
      <DialogHeader>
        <DialogTitle>{promptAction.action === "update" ? "Edit" : "Add"} Prompt</DialogTitle>
      </DialogHeader>
      ...
      <DialogFooter>
        <Button
          disabled={isLoading}
          onClick={promptAction.action === "update" ? handleEditPrompt : handleSavePrompt}
          type="submit"
        >
