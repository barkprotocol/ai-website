...
// import { RPC_URL } from '../constants'; //Removed as RPC_URL is now passed as argument
...
const fetchHelius = async (rpcUrl: string, method: HeliusMethod, params: any) => {
  try {
...
export const getBalance: (rpcUrl: string, walletAddress: string) => Promise<number> = async (
  rpcUrl: string,
  walletAddress: string,
) => {
  const data = await fetchHelius(rpcUrl, "getBalance", [walletAddress])
...
}> = async (rpcUrl: string, ownerAddress: string) => {
  try {
    const data = await fetchHelius(rpcUrl, "searchAssets", {
...
export async function getMintAccountInfo(rpcUrl: string, mint: string): Promise<MintInfo> {
  const data = await fetchHelius(rpcUrl, "getAccountInfo", [mint, { encoding: "jsonParsed" }])
 ...
 */
export async function getTokenHolders(rpcUrl: string, mintInfo: MintInfo): Promise<Map<string, Holder>> {
  let page = 1
  const holderMap = new Map<string, Holder>()

  while (page <= 100) {
    const data = await fetchHelius(rpcUrl, "getTokenAccounts", {
      page,
      limit: 1000,
      displayOptions: {},
      mint: mintInfo.mint,
...
export const getTokenAccountInfo = async (rpcUrl: string, address: string) => {
  const data = await fetchHelius(rpcUrl, "getAccountInfo", [address, { encoding: "jsonParsed" }])
  return data.result.value
}

export async function getTopTokenHolders(rpcUrl: string, mintInfo: MintInfo): Promise<Map<string, Holder>> {
  const data = await fetchHelius(rpcUrl, "getTokenLargestAccounts", [mintInfo.mint])
  ...
  const holderMap = new Map<string, Holder>()
  const tokenAccountsResponse = await getMultipleAccountsInfoHelius(rpcUrl, tokenAccountAddresses)
 ...
 */
export async function getTokenHolderCount(rpcUrl: string, mintInfo: MintInfo): Promise<number> {
 ...
 */
async function getMultipleAccountsInfoHelius(rpcUrl: string, addresses: string[]) {
  return await fetchHelius(rpcUrl, "getMultipleAccounts", [addresses, { encoding: "jsonParsed" }])
}
...
async function classifyAddresses(holderMap: Map<string, Holder>, addresses: string[], chunkSize = 20) {
  ...
  for (const chunk of addressChunks) {
    const response = await getMultipleAccountsInfoHelius(rpcUrl, chunk) //rpcUrl added here
...
export async function getHoldersClassification(
  rpcUrl: string, //rpcUrl added here
  mint: string,
  limit = 10,
) {
  const mintAccountInfo = await getMintAccountInfo(rpcUrl, mint) //rpcUrl added here
  const totalSupply = Number(mintAccountInfo.supply) / 10 ** mintAccountInfo.decimals

  const topHolderMap = await getTopTokenHolders(rpcUrl, mintAccountInfo) //rpcUrl added here
  const totalHolders = await getTokenHolderCount(rpcUrl, mintAccountInfo) //rpcUrl added here
