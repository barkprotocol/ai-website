...
export async function getTokenId(contractAddress: string, network = "solana"): Promise<string> {
  ...
  if (!response.ok) throw new Error(`Failed to fetch token ID for ${contractAddress}`)
...
export async function getPriceHistoryFromCG(
  ...
  if (!response.ok) throw new Error(`Failed to fetch market_chart for ${tokenId}`)
  const data = await response.json()
...
async function getTokenPools(contractAddress: string, network = "solana"): Promise<string> {
  ...
  if (!response.ok) throw new Error(`Failed to fetch token pools for ${contractAddress}`)
...
async function getDexOhlcv(
  ...
  if (!response.ok) throw new Error(`Failed to fetch DEX OHLCV for pool: ${poolId}`)
  const data = await response.json()
