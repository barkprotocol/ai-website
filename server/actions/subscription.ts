...
export const subscribeUser = actionClient.action(async (): Promise<ActionResponse<{ success: boolean }>> => {
  try {
...
export const unsubscribeUser = actionClient.action(async (): Promise<ActionResponse<{ success: boolean }>> => {
  try {
    const authResult = await checkUserAuth()
    if (authResult.success === false || !authResult.user) return authResult
...
export const reactivateUser = actionClient.action(async (): Promise<ActionResponse<{ success: boolean }>> => {
  try {
    const authResult = await checkUserAuth()
    if (authResult.success === false || !authResult.user) return authResult
