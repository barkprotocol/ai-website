...
import type { searchWalletAssets } from "@/lib/solana/helius"
...
export const getTopTraders = cache(
  async ({
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch Birdeye traders: ${response.statusText}`)
      }
      ...
      return parsed.data.items.map((trader) => ({
