...
import { type CoreTool, NoSuchToolError, appendResponseMessages, generateObject, generateText } from "ai"
...
import { dbCreateMessages, dbCreateTokenStat, dbGetConversation } from "@/server/db/queries"
import type { ActionWithUser } from "@/types/db"
...
export async function processAction(action: ActionWithUser) {
  ...
  try {
    ...
    if (!conversation) {
      console.error(`[action:${action.id}] Conversation ${action.conversationId} not found`)
      return
    }
    ...
    if (!activeWallet) {
      console.error(`[action:${action.id}] No active wallet found for user ${action.userId}`)
      return
    }
    ...
    logWithTiming(startTime, `[action:${action.id}] getToolsFromOrchestrator completed`)

    for (const toolName of toolsRequired ?? []) {
      if (toolName.startsWith("INVALID_TOOL:")) {
        console.error(`[action:${action.id}] Unknown tool requested ${toolName}, skipping action`)
    ...
    const messages = await dbCreateMessages({
      messages: finalMessages.map((message) => {
        return {
          ...
          toolInvocations: message.toolInvocations ? JSON.parse(JSON.stringify(message.toolInvocations)) : undefined,
    ...
    logWithTiming(startTime, `[action:${action.id}] dbCreateMessages completed`)
    ...
    if (messages && isValidTokenUsage(usage)) {
      ...
      logWithTiming(startTime, `[action:${action.id}] dbCreateTokenStat completed`)
    }
    ...
    const update = {
      ...
      lastExecutedAt: now,
      completed: !!action.maxExecutions && action.timesExecuted + 1 >= action.maxExecutions,
    ...
    if (!successfulExecution && action.lastSuccessAt) {
      ...
      if (lastSuccessAt.isBefore(oneDayAgo)) {
        ...
        console.log(`[action:${action.id}] paused - execution failed and no recent success`)
      ...
      if (action.timesExecuted >= ACTION_PAUSE_THRESHOLD) {
        ...
        console.log(`[action:${action.id}] paused - execution failed repeatedly`)
