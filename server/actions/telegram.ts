...
const getChatIdByUsername = async (username: string): Promise<string | null> => {
  const response = await fetch(GET_UPDATES_URL)
  if (!response.ok) throw new Error("Failed to retrieve bot updates")
  const data = await response.json()
  const chat = data.result.find((msg: any) => msg?.message?.from?.username === username)
...
async function checkUserTelegramSetup(username?: string, userId?: string): Promise<CheckUserSetupResult> {
  ...
  const chatId = userTelegramChat?.chatId ?? (await getChatIdByUsername(finalUsername))
...
export const verifyTelegramSetupAction = actionClient
  ...
  .action<ActionResponse<TelegramActionData>>(async ({ username, userId }) => {
    try {
      const setup = await checkUserTelegramSetup(username, userId)
      if (!setup.success) {
        return {
          success: false,
          error: setup.error,
...
export const sendTelegramNotification = actionClient
  ...
  .action<ActionResponse<TelegramActionData>>(async ({ username, userId, text }) => {
    ...
    try {
      ...
      await sendTelegramMessage(setup.chatId!, text)
